...
// --- Definições para o Sensor AHT10 ---
...
// --- Implementação das Funções do OLED ---
...
int main()
{
    ...
    while (true)
    {
        if (aht10_read_data(&humidity, &temperature))
        {
            ...
            // Prepara as strings para o OLED (sempre as leituras atuais)
            snprintf(temp_str, sizeof(temp_str), "Temp: %.1f C", temperature);
            snprintf(hum_str, sizeof(hum_str), "Umid: %.1f %%RH", humidity);

            clear_oled_display(); // Limpa o display antes de desenhar novas leituras

            display_message_oled(temp_str, 0); // Exibe temperatura na linha 0
            display_message_oled(hum_str, 2);  // Exibe umidade na linha 2

            // *** AQUI É ONDE O ALERTA É EXIBIDO NO OLED ***
            if (alerta_str[0] != '\0')
            {                                        // Verifica se a string de alerta NÃO está vazia
                display_message_oled(alerta_str, 4); // Exibe o alerta na linha 4 (dando um espaço)
            }
